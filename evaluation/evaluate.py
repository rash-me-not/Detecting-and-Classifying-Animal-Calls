...
model = load_model(
    r'/mar/home/rmishra/PycharmProjects/Detecting-and-Classifying-Animal-Calls/saved_models/model_2019-12-21_21_02_27.534641_network_train/savedmodel.h5')

dataset_dir = "/cache/rmishra/dataset"
x_test_aud = np.load(os.path.join(dataset_dir, "dataset_aud/x_test.npy"))
x_test_acc_ch0 = np.load(os.path.join(dataset_dir, "dataset_acc_ch_0/x_test.npy"))
x_test_acc_ch1 = np.load(os.path.join(dataset_dir, "dataset_acc_ch_1/x_test.npy"))
...
def plot_focal_pred_per_call_type(aud, foc, class_names_audio, class_names_foctype, plot_title, saved_fig_title):
...
plot_focal_pred_per_call_type(y_test_aud[:, :, :8], y_test_foc, class_names[0], class_names[1],
                              "Ground Truth: Focal/Non-Focal/Unknown call per Vocal Call type", "FocPerCallTypeGt.png")
plot_focal_pred_per_call_type(y_test_aud[:, :, :8], preds_w_threshold[1], class_names[0], class_names[1],
                              "Predicted Focal/Non-Focal/Unknown call per Vocal Call type (Ground Truth)",
                              "FocPerCallType_GtAud_PredFoc.png")
plot_focal_pred_per_call_type(preds_w_threshold[0][:, :, :8], y_test_foc, class_names[0], class_names[1],
                              "Focal/Non-Focal/Unknown call(Ground Truth) per Predicted Vocal Call type",
                              "FocPerCallType_GtFoc_PredAud.png")
plot_focal_pred_per_call_type(preds_w_threshold[0][:, :, :8], preds_w_threshold[1], class_names[0], class_names[1],
